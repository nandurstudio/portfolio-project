...
export default function PembayaranAngsuranAdmin() {
  ...
  const fetchInstallments = async () => {
    setIsLoading(true);
    try {
      const data = await api.getAdminInstallments();
  ...
  const handleAction = async (id: number, status: 'approved' | 'rejected') => {
    if (!confirm(`Apakah Anda yakin ingin ${status === 'approved' ? 'menyetujui' : 'menolak'} angsuran ini?`)) {
      return;
    }
    setIsVerifying(true);
    try {
      await api.verifyInstallment(id, status);
      alert(`Angsuran berhasil ${status === 'approved' ? 'disetujui' : 'ditolak'}!`);
      fetchInstallments();
    } catch (err: any) {
  ...
  const filteredData = installments.filter(
    (item) =>
      item.user?.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.loan_id?.toString().includes(searchTerm)
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="mb-8">
          <h1 className="text-2xl text-gray-900 mb-1">Verifikasi Pembayaran Angsuran</h1>
          <p className="text-gray-500">Validasi pembayaran angsuran dari anggota</p>
        </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-sm text-gray-600 font-medium">ID Pinjaman</th>
                  <th className="px-6 py-3 text-left text-sm text-gray-600 font-medium">Nominal Angsuran</th>
                ...
                {filteredData.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-6 py-8 text-center text-gray-500">
                      Tidak ada pengajuan angsuran ditemukan
                    </td>
                  </tr>
                ) : (
                  filteredData.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900">P-{item.loan_id}</td>
